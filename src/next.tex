...
\par
The standard and turbo version only weakness remained the video. Shipping with 256 KiB of VRAM the machine could only output four colors (white, black and two shades of gray). To bring color to the NextCube, customers had to invest in a NeXTDimension board.\\
\par
...
To select the i860 as the main chip of a graphic card could have been called overkill but it was a careful and ultimately sound decision. Video processing is an intensive CPU task. It benefits so much from the i860 SIMD pipeline that nothing else but Intel's "Cray on a Chip" could have done the job.\\
\par
\fq{
...
\par
NeXTSTEP was used as foundation for Apple's Rhapsody project which became Darwin, the core of Apple's OSes. The new operating system was met with enthusiasm by customers and professionals who praised the design, look and feel, and stability. Darwin was later used as a base for Apple's 2008 iPhone which took over the world by storm. It is extremely likely some of the code that once ran on NeXTSTEP and contributed to \doom{} now runs on the many millions of Apple computers and iOS phones across the world.\\
