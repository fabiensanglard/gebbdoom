...
Since cost was the main issue with their product line, NeXT attempted to introduce less expensive product. They designed something close to the NeXTCube but removed non essential elements in order to produce a three times cheaper, all-in-one machine.\\
\par
...
\par
\trivia{Notice on the picture that no button or switch are visible on the computer itself. The machine could only be turned on and off via the keyboard (a novelty at the time). An update later introduced the Apple Desktop Bus created by Steve Wozniak which bear many similarities with the USB released in 1996.}
\pagebreak
