...
\vspace{5pt}
\cscaledimage{0.95}{DTL-H2000-CPU1and2.png}{DTL-H2000 dual ISA card devkit. SCSI HDD and CD-ROM burne not shown}
\vspace{-10pt}
\par
...
\end{wrapfigure}
It was initially difficult to convince developers to get on board and work with the PlayStation. On October 27, 1997, Sony gathered 300 developers representing 60 games publishers for a tour-de-force. They were shown the "dino demo"\footnote{Source: "PlayStation: Anthology"} which features a real-time controllable T-rex dinosaur.\\
...
\par
  The restrictions did not have to be as drastic as for Atari's console. Thanks to the CD-ROM capacity, 59 maps (30 from \doom{} and 30 from \doomii{}) shipped. To compensate for the slow access time and the restricted amount of RAM, each map is stored in its own \cw{WAD} archive. On the other hand, most monsters are present except for the Arch-Vile.\\
\par
...
From the previous screenshot we can see that something is wrong but it is not clear what. There is obviously a distortion but it is not exactly what we saw on page \pageref{affine_texture_examples}. The difference is that we were able to draw a quad directly whereas the Playstation's GPU is only capable to process triangles.\\
\par
...
\par
 To rasterize these triangles the GPU has no choice but to use a scanline algorithm. The process preserve line parallelism and moreover there seems to be no "agreement" between the two triangles which result in unpleasant "zigzag".\\
\par
...
% \par
In figure \ref{texture_anmgle}, the visual artifacts becomes exacerbated as the angle of the wall increases. Also notice in the left column how the width of each square is always constant, a giveaway of affine texturing which contrast with the perspective correct decreasing width seem on the right column.\\ 
\pagebreak
...
Sony was well aware of the problem but manufacturing cost prevented gifting the PlayStation with perspective correct hardware (they also lacked a partner with strong computer graphics experience such as SGI which deeply influenced the Nintendo 64). The PSX's developer manual recommended way to mitigate the problem was to subdivide triangles into more triangles.\\
\par
 That may sound like running away from the problem but the PSX was capable to process a rather high number of triangles for its time so it was not a bad suggestion. To get a satisfactory visual result however, the number of triangles had to be high.\\
\par
