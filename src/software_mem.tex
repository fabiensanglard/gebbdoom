...
The memory manager has many other features over libc's \cw{malloc}.\\
\par
\begin{itemize}
\item The field \cw{id} is used as a canary marker to detect memory overflow (the value should always be \cw{ZONEID}). 
\item \cw{Z\_Free} is able to detect mismanagements such as double free. 
\item A dump system accessed via \cw{Z\_DumpHeap} allow memory inspection.
\item An integrity checker \cw{Z\_CheckHeap} which verify all blocks "touch" each others or that there are no two consecutive free blocks.
\end{itemize}
