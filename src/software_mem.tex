...
\ccode{memory_zone.c}\\
\par
In its initial start, all ram is in a free block. To make the drawing simpler, let's assume the entire ram is 8000 bytes. The block has been marked \cw{STATIC}. It has a \cw{NULL} \cw{user}. Its size is 8000. Both \cw{next} and \cw{prev} point to itself. The rover point to the only block in existence.\\
\par
...
\par
\trivia{Blocks can be "retagged" via function \cw{Z\_ChangeTag}. It is frequent for the engine to allocate memory blocks with a \cw{PU\_STATIC} tag while working with assets only to change the tag to \cw{PU\_CACHED} after it is done with it. This allow memory to be freed if needed while potentially avoiding a round trip to the HDD if the block is still in RAM.}
