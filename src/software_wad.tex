...
\begin{tabularx}{\textwidth}{ L{0.2}  L{0.8}}
  ...
  \cw{SIDEDEFS} & All sides referenced by \cw{LINEDEFS}. A line can have up to two sides.\\
  ...
  \cw{SECTORS} &  Referenced by \cw{SSECTORS}. Details ceiling/floor height, texture, and lighting properties.\\
...
\par
In this illustration, \cw{DOOM1.WAD} is represented with only three lumps and the two additional WAD archives have only one lump each. In the following sequence, see how one lump name can appear several times (there are two lumps named \cw{MUSIC1}) in the index.\\
\par
...
\par
The beauty of this system means that the original \cw{DOOM1.WAD} never had to be modified or patched. Any of the assets could be overwritten -- from maps, music, sfx, to graphics\footnote{With the exception of A.I. which is stored in the executable} -- with a simple command-line.\\
\par
