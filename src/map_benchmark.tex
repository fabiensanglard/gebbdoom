...
\doom{} on a NeXTStation screen. Usually the \cw{TextEdit.app} window would be on the right. Notice "Interceptor VGA Console" which comes from \cw{libinterceptor}, a private library provided by \NeXT{}'s engineers allowing a hole to be punched in Display Postscript to speed up rendering. Resolution was so high that id had to implement a 2x zoom to see the pixels.\\
\par
