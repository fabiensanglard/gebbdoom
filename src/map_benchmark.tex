...
\centering  
\begin{tabularx}{\textwidth}{ L{0.08} | R{0.42} | R{0.25} | R{0.25} }
  \specialrule{1pt}{0pt}{0pt}
...
\end{figure}
\par
It gets even worse when running the game with the 2x zoom used during development. In this mode, the same number of pixels are written to the core's framebuffer but 4x times more data has to transit over the bus.\\
\par
 \begin{figure}[H]
\centering  
\begin{tabularx}{\textwidth}{ L{0.08} | R{0.21} | R{0.21} | R{0.25} | R{0.25} }
  \specialrule{1pt}{0pt}{0pt}
  \textbf{Mode} & \textbf{Pixels written} & \textbf{Pixels moved} & \textbf{High Details FPS} & \textbf{Low Details FPS} \\
  \specialrule{1pt}{0pt}{0pt}
B & 64,000 & 256,000& 6 & 8 \\  
A & 53,760& 215,040 & 6 & 8 \\
9 & 41,472& 165,888 & 7 & 9 \\
8 & 32,768& 131,072 & 8 & 9 \\
7 & 25,088& 100,352 & 8& 10\\
6 & 18,432&  73,728& 9 & 10 \\
5 & 12,800&  51,200& 9 & 10 \\
4 &  8,192& 32,768& 10 & 11 \\
3 &  4,608& 18,432& 11  & 11 \\
   \specialrule{1pt}{0pt}{0pt}
\end{tabularx}
...
\par
 The implementation disregard   s update signals from \cw{I\_UpdateNoBlit} and defers all work to \cw{I\_FinishUpdate} where the full content of framebuffer \#0 is blitted to \cw{NSWindow}. There is no dirty box optimization and no direct access to the hardware like on DOS.\\
\par
\trivia{The "low detail" mode was never properly implemented on NeXTSTEP. The engine writes only half the columns but there is no system to duplicate the columns like the VGA mask does. As a result, only the left portion of the screen is updated.}
...
\par
\trivia{Many ports got the aspect ratio wrong. \doom{} 95 which was to showcase Microsoft's Windows 95 graphics drivers low overheard was among the guilty. In its default setting, the 320x200 hosting window directly maps the \doom{}'s core framebuffer. As a result, enemies look shorter, rocket explosions are oval, and everything else is distorted.}
\par
