...
Under DOS the normal way to perform a "system call" is to use a software interrupt instruction with parameter \cw{21h}. In C programming, this was abstracted away by header \cw{DOS.H} which performed all the lower level work behind the scene.\\
\par
...
Upon startup, the DOS Extender would place hooks in the OS software Interrupt Vector Table and place its own routines there. From the application standing point everything was transparent, the developer had no code to change. To perform a system calls not hooked by the extender (e.g: \cw{int 33h} to read mouse inputs), the extender offered a special interface called DPMI on interrupt \cw{31h} which took care of translating 32-bit register request to 16-bit so IVT routines would understand them.\\
\par
\trivia{DMPI (DOS Protected Mode Interface) was originally created to allow Windows 3.0 to run 32-bit application and be compatible with a join operating system project with IBM called OS/2.}\\
\par
