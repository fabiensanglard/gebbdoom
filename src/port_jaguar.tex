...
\end{wrapfigure}
Development of the machine started in 1990 when Atari commissioned Cambridge based Flare Technology to design not only one but two new game system simulaneously. A fourth generation, 32-bit, system called Panther and an audacious 64-bit system called Jaguar\footnote{Atari named all its console after big cats. Besides the Jaguar and Panther, its handheld game system from 1989 was called "Lynx".} were to be developed.\\
...
\par
\trivia{To ease the task of generating RISC instructions for Tom \& Jerry, John Carmack wrote its own \cw{lcc} compiler backend. Ouptut were optimized further by hand.\footnote{It was not the last time id Software would use the excellent \cw{lcc}. in 1999, it was used for Quake 3 to generate the VMs bytecode.}}\\
\par
