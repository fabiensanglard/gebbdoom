...
\par
There is one blockmap per map which was generated via \cw{doombsp} preprocessing on NextStations. Saved in a lump audaciously named \cw{BLOCKMAP}, it is used a runtime to lower the number of lines to test intersections with.\\
\par
...
All map traversal are done with an abstract method \cw{P\_PathTraverse} which takes as argument what to check collision with and a function pointer to call upon hit.\\
\par
\ccode{P_PathTraverse.c}
\par
Function \cw{P\_AimLineAttack} which is used for punching and sawing uses \cw{P\_PathTraverse} with flag = \cw{PT\_ADDLINES|PT\_ADDTHINGS}. The block coordinates of any map coordinate is easy to obtain via modulo 128. Fast 128x128 grid intersections is also fast thanks to DDA algorithm\footnote{DDA Algorithm was detailled in "Game Engine Black Book: Wolfenstein 3D".}.  To detect Things collisions, their positions are tracked in blocks.\\
\par
