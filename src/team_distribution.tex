...
\par
\cw{DOOM.EXE} had a command-line parameter allowing modders to load their own \cw{WAD}s in order to overwrite \cw{DOOM.WAD} lump entries. This mechanism permitted to customize almost everything. A custom \cw{WAD} containing an \cw{E1M1} lump could be used via a simple \cw{doom -file mylevel.wad} command (detailled on page \pageref{wad_detailled}).
