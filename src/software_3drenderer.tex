...
\par
The beauty of binary trees is that traversing them is $O(log\ n)$. No matter where we try to place the player on this map, it will never take more than three tests to sort all subsectors and their segments.
\pagebreak
...
To perform the side test (\cw{R\_PointOnSide)}, any geometry book will describe how to represent the plane in general form with a vector $(a, b, c)$ combined to a distance $d$: $$ ax + by + cz + d = 0 $$
and use the power of the dot product to inject the coordinate of the test point $P = (x, y, z)$, essentially projecting $P$ onto a line perpendicular to the plane. The sign of the result reveals whether the point is in front of or behind the plane (zero=onplan).\\
\par
