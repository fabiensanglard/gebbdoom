...
\par
This method is far from being optimal. There is a better way, involving neither floating-point nor fixed-point arithmetic. It uses not the dot product but the cross product.\\
\ccode{node_t.c}\\
\par
\vspace{2mm}
...
\begin{wrapfigure}[15]{f}{0.4\textwidth}
\centering
...
\par
This effect was diffcult to replicate with a seemingly "more powerful" hardware accelerated systems which uses 24-bit colors. On iOS, \cw{glBlendFunc(GL\_ONE\_MINUS\_DST\_COLOR, GL\_ZERO)} was used to approximate the same visual with mixed results.\\
