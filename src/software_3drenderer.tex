...
Using a dot product operation, the coordinate of the test point $P = (x, y)$ is injected into the plan equation, essentially projecting $P$ onto a line perpendicular to the plane. The sign of the result reveals whether the point is in front of or behind the plane (zero = on the plane).\\
\par
...
\par
Despite being stored as columns, visplanes are converted to horizontal spans and drawn as such. This is done this way because each line is at a constant distance from the player which allows for fast perspective correct texturing 
...
This choice also allowed fast rendering of something we have so far ignored as a matter of simplicity: diminished lighting.
