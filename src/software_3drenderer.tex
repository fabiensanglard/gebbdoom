...
\par
 To illustrate the concept, let's study what affine texture mapping (incorrect) looks like in a simple scene featuring three walls textured with a pattern of white and colored squares.\\
\par
...
\par
Given the previous screenshots, it may not be vivid why perspective correctness is so important and why the engine goes to such an extend just to be "correct". Keep in mind these were just examples to demonstrate the problem. As soon as real textures are used, the visual disturbance cannot be ignored.\\
\par
In the scene below, notice how the beast head is weirdly distorted. Same comment for the wall on the right. Despite the horizontal line being perspective correct, the texturing does not provide the illusion of perspective. On the opposite page, the scene is correct.\\
\par
\fullimage{game_affine_texturing.png}
\trivia{Several video game consoles of the era such as the PlayStation, the 3DO, and the Saturn featured hardware accelerated texture mapping but not implemented perspective correctness. On these machines, affine texture mapping artifacts were supposed to be compensated via triangle sub-divisions. id Software vetoed all ports attempting to be perspective incorrect which sometimes had pretty serious consequences (see page \pageref{XXX} about Saturn port).}\\
\par
Perspective ... \\
\par
...
Above shows the vanilla engine. Below is the same scene with a modified lightmaps-less engine.
\vspace{2mm}
