...
\end{itemize}
 Its most breathtaking characteristic is its ability to render walls and flats with zero overdraw. Sprites and Transparent walls do introduce a little bit of overdraw but it is minimal.\\
\par
...
\par
\trivia{The engine stores visplanes before drawing them. If storage runs out, the engine terminate execution and returns to DOS with a less than explicit error message.}\\
\par
...
\par
Since distance from the player is constant over an horizontal line, visplanes are rendered as spans (instead of columns) for perspective correctness. This choice also allowed fast rendering of something we have so far ignored as a matter of simplicity: diminished lighting.
...
\par
\cfullimage{masked_nomasked.png}{Same scene as it is currentyly stored in the framebuffer}
\par
