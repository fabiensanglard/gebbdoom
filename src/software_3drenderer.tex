...
With the expert knowledge of BSP in mind, let's take a look at the first step of a scene rendition. Namely, to render walls. As expected, \cw{R\_RenderBSPNode} traverses the binary tree front to back. Each sub-sectors leaves are sent down to the renderer via \cw{R\_Subsector}.\\
\par
...
\vspace{-4mm}
After all this clipping, walls and portal screenspace coordinates are finally render. For each fragment ends, an \cw{Y} offset is calculated based on sector floor and a column height is generated based on floor/ceiling and distance. These are interpolated to generate a set of full columns of pixels (portals are drawn as a combination of columns according to their upper texture, middle texture, and upper texture while walls are only middle texture). Rendition is done vertically via \cw{colfunc} function pointer (detailed in the "performance" page \pageref{performances}).\\
...
\vspace{-4mm}
\trivia{Visplane rendition is deferred. They are stored while walls were rendered. 664 bytes per visplane (and 128 visplanes) represent a significant amount of RAM (84,992 bytes) for a game limited to 4MiB.}\\
\par
...
\par
Lightmap \cw{COLORMAP} contains a 32th column made of 256 shades of grey. It is used when the player picks up the invulnerability bonus. There is a bug in the visplane rendition routine special case in charge of skies. Skies are always renderered without diminished lighting and therefore ignore lightmaps.\\
\par
