...
This architecture explains why sound memory allocation had a special treatment in the zone memory allocator. Because the audio system runs in an interrupt, each interruption must be acknowledged as fast as possible. This system simply had no time to recover from a memory miss. Audio data had to be ready at all time while it could be used.\\
\par
...
\ccode{dmx_before.c}
\par
\doom was supposed to emit sounds at random pitch to avoid monotony. The DMX API was modified to accommodate this need in vXXX:\\
\par
\ccode{dmx_after.c}
\par
...
\par
TODO: Dig inside article "The dark and forgotten art of GENMIDI".
