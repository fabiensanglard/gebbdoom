...
\par
The version shipping with NeXT computers featured an important library called Foundation Kit. One of its components, \cw{NSObject}, freed developers from the error-prone memory management burden by offering reference counting via its \cw{retain} and \cw{release} methods. A collection of swiss army knife containers -- \cw{NSArray}, \cw{NSDictionary}, \cw{NSSet} and \cw{NSData} -- once again allowed focusing on the core functionality instead of losing time on infrastructure.\\
\par
\trivia{Initially using the prefix NX, all objects in Foundation were renamed with a leading NS for OpenSTEP where "NS" stands for the alliance between NeXT and Sun Microsystems. All these objects are still at the core of Mac OS X and iOS today; the prefix NS was never removed.}\\
\par
