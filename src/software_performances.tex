...
There is a convenient and portable way to assess performances. Thanks to the fixed tic duration architecture, a demo can be recorded and replayed with exactitude. With frame skipping disabled, a \cw{timedemo} (as it is called) will produce the exact same sequence of frames to render regardless of the machine architecture and power.\\
\par
...
The first value, \cw{gameticks}, is the number of frames encountered. For \cw{demo3} it is always equal to 2134 since it comes from the record lump. The second value, \cw{realticks}, represents the walltime in ticks. It represent how fast the machine managed to replay the record.\\
\par
...
{
 $ fps = \frac{walltime}{realticks} * 35 $ 
 }
 \end{equation*}\\

 \par
In the example above\footnote{On a miniPC Unisys CWD 4001 (486DX2-66/CirrusLogic-GD5424).}, the game ran at an average of $\frac{1065}{2134}*35 = 17$ fps. \\
\par
...
\par
 As demonstrated, none of the hardware of the time was able to visually max out the game. Beyond 35fps there would have been no visual improvement since the game logic is running at 35fps. A machine able to run at 70fps would simple render the same frame twice since the game logic would only move forware every two frames.\\
\par
\begin{figure}[H]
\centering  
...
Even without any special tools, it is possible to gain insight on which part of the engine are responsible for CPU cycles consumptions thanks to built-in command line parameters.\\
\par
...
\begin{tabularx}{\textwidth}{ C{1} C{1} C{1} C{1} } 
  ...
  \toprule 
320x200  &  19   & 160x200  &       30 \\
320x168  &  XX   & 160x168  &       XX \\
288x144  &  XX   & 144x144  &       XX \\
256x128  &  XX   & 128x128  &       XX \\
224x112  &  XX   & 112x112  &       XX \\
192x096  &  XX   & 096x096  &       XX \\
160x080  &  XX   & 080x080  &       XX \\
128x064  &  XX   & 064x064  &       XX \\
...
\par
Notice the linear 2x performance improvement regardless of the resolution used.
...
\begin{tabularx}{\textwidth}{ R{1}  R{1} R{1} R{1}  R{1.4}  R{0.6} }
 ...
 B & 320 & 200 & 64,000 & 100 & 19 \\
 A & 320 & 168 & 53,760 &  84 & 20 \\
...
\par
The trade off gain\footnote{Benchmarked on a miniPC Unisys CWD 4001 (486DX2-66/CirrusLogic-GD5424, 8MiB RAM).} was not as substantial as the high/low resolution. Reducing the 3D canvas size by 50\% from 11 to 8 yields only a 31\% improvement yet the visible area is so small it its almost not worth it.\\
\footnotetext{limit of playability}



\par
\trivia{To play nicely with the i486 cachelines (at least for read operations), texture data are stored as column major (rotated 90 degrees left).}\\
\fixme{Texture and rotated texture go here.}
