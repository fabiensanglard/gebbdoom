...
\par
The DOS flame graph shows that I.A. ran in \cw{G\_Ticker} consumes little CPU time. The audio routine \cw{S\_UpdateSounds} is even less taxing since it is barely visible as the tiny red span. This makes sense since the only work it does is to retrieve sound effects and music data from the \cw{.wad} and place it into RAM for the audio card to use.\\
\par
