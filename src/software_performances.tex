...
\par
 Something weirder is to notice that \cw{I\_FinishUpdate} is not visible from the flamegraph, which means it is almost instantaneous. How can the DOS version transport a full framebuffer across the bus so fast? Because it doesn't! The DOS version benefited from heavily optimization.\\
\par
