...
Let's dive into the text code right away and take a look at the section of \cw{multigen.txt}  which describe the Imp (known as \cw{TROOP} internaly).\\
\par
\tcode{enemy_info.txt}

\tcode{imp_state.txt}

There are four sections. Properties such as DoomED id, \cw{speed}, \cw{heigh}, \cw{radius}. State names for state machine targets. Sound strings. And finally the huge Action definition.
...
\par
\drawing{E1M1_sides_with_player}{A simple example with E1M1 (skill level 3) allows to drop four monsters from the collision detection set.}
