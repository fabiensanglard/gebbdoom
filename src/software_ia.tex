...
% \par
% \pngdrawing{automate}{}
Figure \ref{reject_explained} features three sectors with five monsters and one player. The engine needs only to run one monster's I.A. seeking for the player since the later is in sector \circled{1}. All four others monsters are "rejected" until the player most inside sector \circled{3}. The table is only used for sight. Monsters will still hear the player since sound is cheaper to propagate.\\
\par
