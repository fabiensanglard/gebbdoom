...
% \par
% \pngdrawing{automate}{}
Figure \ref{reject_explained} features three sectors with five monsters and one player. The engine needs only to run one monster I.A seeking for the player since the later is in sector one. All four others monsters are "rejected" until the player most inside sector three. The table is only used for sight. Monsters will still hear the player (sound is cheaper to propagate).\\
\par
...
\end{wrapfigure}
Who doesn't recognize E1M3 hunt for the blue key on page \pageref{e1m3_trap}. After traversing the whole map, it is finally there enlighted on a pedestral. Guarded by only two low levels opponents which are easily dispatched.\\
\par
...
Finally, the blue key (above)! Nooo, it's a TRAP (below)!\label{e1m3_trap}
