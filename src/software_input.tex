...
\ccode{events.c}
\par
The core system notifies the input system when a new frame starts or when a game tic starts which gives it an opportunity to sample devices for inputs, wrap them into an \cw{event\_t} and use a callback function to post it in the core event buffer.\\
\par
\begin{figure}[H]
\centering  
...
\par
This feature allowed \doom{} to have demos like you could find in arcades. The \cw{DEMO1}, \cw{DEMO2}, and \cw{DEMO3} lumps are streams of \cw{ticcmd\_t}s meant to be played at 35Hz. Since only commands are stored and the game is deterministic, demo files are very small, consuming only $ 128 * 35 = 4480 $ bytes/second.\\
% \par
