...
\par
\par
\cscaledimage{1}{i486DX.png}{The Intel 486 packing. Its 1.0$\mu$ process allowing 1,180,235 transistors.}
\par
\trivia{The i860 played a part in \doom~anyway since it was used in NextDimensions.}
...
\par
\scaleddrawing{0.5}{DRAM}{Dynamic RAM made of two elements}
...
\par
\vspace{2mm}
\scaleddrawing{0.7}{SRAM}{Static RAM made of six elements}
\par
...
\scaleddrawing{0.9}{set_state_caches}{}
...
\par
\drawing{netburst}{NetBurst allows for 65\% faster cacheline filling.}
...
\par
Albeit a few peculiarities\footnote{"Pushing the 486" by Michael Abrash.}, as long as the programmer was mindful of the cacheline and maximized time and space locality\footnote{And avoid branching. Without a branch predictor, \cw{jmp} are ignored and usually incur a two cycles stale.}, the CPU would fly to process integers. Floating point operations had improved a lot (the i487 FPU can \cw{FMUL} faster than the i386 can \cw{IMUL}) and floating point operations performances had improved by a factor 2 compared to the i386's FPU (i387).\\
\par
\begin{figure}[H]
\centering
...
\end{figure}
\par
For \doom, there was not the shadow of a doubt, it had to be done with integer operations\footnote{The dawn of floating points would beging with Intel's Pentium and Quake in 1996.}.\\
% \par
