...
\par
If in retrospect the 486 was Intel's 1994 champion and an unquestionable powerhouse (both in terms of performance and sales\footnote{As of late 2015, the 486 was still manufactured and used inside network routers.}), it had to sustain a period of uncertainty. Just as the 386 had to face its brother (the i960), the i486 also had to a competitor from the same company. The competing brother was named the "Intel 860".\\
\par
\fq{
...
On paper, the i860 was impressive and a serious competitor. Relying on an heavily pipelined super-scalar architecture crushing VLIWs\footnote{Very Long Instruction Word.}. It had three units, X, Y, and Z allowing parallel processing and when used efficiently could outperform the Intel 486.\\
\par
...
\par
Figure \ref{i486DX.png} shows the Intel 486 die and the 1,180,235 transistors inside its package. Around this time period, Intel started to stamp its CPU with trademarked logo in an attempt to distance itself from increasingly aggressive AMD and Cyrix clones.\\
\par
...
\par
Even if the Prefetch Unit and the Execution Unit were properly fed, the Decode unit always took a minimum of two cycles to decode an instruction\footnote{The author speculates this high decode cost was the result of Intel's choice to use CISC instead of RISC.}. Since the maximum throughput of a pipeline cannot exceed the speed of its slowest stage, the Intel 386 could process at most one instruction every two cycles.\\
\par
...
\par
It was a difficult constraint to fulfill for physical reasons. Since 1980, RAM performance had been lagging behind CPU performance. Each year, CPU improved by 60\% while DRAM had only improved by 7\%, the gap increasing by 50\%/year. By 1989, DRAM access time was 10 times slower than CPU cycle time.\\
\par
\vspace{2mm}
\drawing{ram_vs_cpu}{}
\par
...
\par 
A first cycle initialized the bus request, placerd the address on the address line and set the control line (Read/Write). A wait cycle (which Intel called Wait State since while waiting on the Bus Unit, the CPU did absolutely nothing) paused while the device on the other side of the bus fulfilled the request.\\
\par
...
\par
\trivia{What about increasing the number of ways? What about the cache size? With 8 KiB of cache, four ways grant the best trade-off\footnote{Source: "Computer Architecture: A Quantitative Approach" by Hennessy/Patterson.}. A two-ways cache yields a 14\% miss rate, four-ways give a 10.5\% hit rate but going to eight only improves the percentage to 10\% and fully associative to 9\%. The decreasing return on investment appears vividly once charted on a graph.}\\
\par
...
\footnotetext{Source: "Roy Longbottom's PC Benchmark Collection: http://www.roylongbottom.org.uk/mips.htm".}
\par
On the chart above, notice how a 486DX2-66Mhz is faster than a 485DX-50Mhz but not the full 20\% frequency could have made us expect. This is because the DX2 bus runs at 33Mhz while on the DX, both the CPU and bus run at 50Mhz.
...
\par
But the FPU performance was still a far cry compared to the the ALU and its barrel shifter. This mandated \doom{} to use integer operations exclusively\footnote{The dawn of floating-point would begin with Intel's Pentium and Quake in 1996.}.\\
...
\begin{tabularx}{\textwidth}{ L{0.3} L{0.3} L{0.4}}
  ...
  \toprule 
   ...
   \toprule
\end{tabularx}
\caption{Comparison of ALU vs FPU operations.}
\end{figure}
\par
