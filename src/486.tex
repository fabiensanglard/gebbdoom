...
\par
\par
\cscaledimage{1}{i486DX.png}{The Intel 486 packing. 1.0$\mu$ processed allowing 1,180,235 transistors.}
\par
...
\par
\scaleddrawing{0.5}{DRAM}{Dynamic RAM}
...
\par
\vspace{2mm}
\scaleddrawing{0.7}{SRAM}{Static RAM}
\par
...
\par
Albeit a few peculiarities\footnote{"Pushing the 486" by Michael Abrash.}, as long as the programmer was mindful of the cacheline and maximized time and space locality\footnote{And avoid branching, the 486 had no branch predictor. A \cw{jmp} instruction was a guaranteed pipeline flush.}, the CPU would fly to process integers. Floating point operations had improved a lot (the i487 FPU can \cw{FMUL} faster than the i386 can \cw{IMUL}) and floating point operations performances had improved by a factor 2 compared to the i386's FPU (i387).\\
\par
\begin{figure}[H]
\centering
...
\end{figure}
\par
For \doom~, there was not a doubt, it had to be done with integer operations\footnote{The dawn of floating points would start with Intel's Pentium and Quake in 1996.}.\\
% \par
