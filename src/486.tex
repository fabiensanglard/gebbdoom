...
\end{figure}
\par
Doom had no choice, it had to be programmed with integer operations\footnote{The dawn of floating points would start with Intel's Pentium and Quake in 1996.}.\\
\par
\trivia{Re-read Michael Abrash guide on 486. Add more insight on how to pipeline the 486 well\\.}\\x
\par
\par
The difficulty to access information gave birth to many myths. One endless thread which occurred on \cw{alt.games.doom} in 1994 summarize helps to appreciate the state of things. The topic "Does a 486DX run Doom faster than an SX?" from July 1994 and its five answers shows how difficult it was to reach the truth (or hard facts :) !).\\
 \par
