...
Notice how the first pixel \cw{0} is stored in bank 0, pixel \cw{1} is stored in bank 1 and so on. With a horizontal resolution of 320 columns, 80 non-horizontally adjacent yet vertically adjacent pixels are stored in each bank.\\
\par
...
\par
\vspace{-2mm}
Another diffculty came from the differing aspect ratio of the mode Y framebuffer layout and the CRT display which resulted in distortion. \\
\par
