...
\par
The card above was manufactured by Britek Electronics. By licensing the ET4000 (\circled{1}), they only had to provide RAM(\circled{2}), RAMDAC(\circled{3}), a Timer(\circled{4}) and apply a few customization via a Tibpal (\circled{5}). The VGA BIOS chip (\circled{6}) could only be purchased from Tseng Labs.\\
\par
...
\par
\trivia{The close tie of the VL-Bus architecture to the architecture of the 486 Bus brought unmatched performances. But it can at at price. When Intel released its Pentium, the Bus Unit protocol had changed (to match Intel's PCI protocol) and the VL-Bus found itself obsolete and unable to compete. The standard died shortly after.}\\
\par
