The PC environment had morphed significantly between the development of Wolfenstein 3D in 1991 and the development of Doom in 1993. The previous "recommend configuration" made of an Intel 386 machine with 2~MiB of RAM and a VGA graphic card was no more.\\
\par
The "new" typical PC still had seven subsystems \circled{1}~Inputs, \circled{2}~CPU, \circled{3}~RAM, \circled{4}~Bus, \circled{5}~Video~output, \circled{6}~Audio~output and \circled{7}~Operating~System together forming a pipeline.\\
\par
\pngdrawing{pc_components}{The seven components of an IBM PC.}
\par
...
\cfullimage{ibm_ps1_top}{IBM PS1 featuring an Intel 486. Notice how the ad emphasis static screen office apps. Also notice the standard 14" CRT which probably waited close to 20 pounds!}
...
\par
The RAM system had grown in capacity, speed and complexity. Thanks to a sharp decline in manufacturing price, the standard DRAM installed (\circled{4}) had doubled to 4MiB\footnote{\doom would not run on PC equipped with "only" 2 MiB}.\\
\par
...
\end{tikzpicture}
\end{center}
A new type of RAM also found its way into these new PC. In black (\circled{5}), eight chips of Static RAM offered 256 KiB acting as "cache", a new system to prevent CPU starvation.
% \begin{enumerate}
