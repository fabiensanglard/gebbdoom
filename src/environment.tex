...
\item A price tag where a top of line machine fetched close to \$3,000. To compare with the competitions, the SNES and the SEGA Genesis were both priced at US\$199 while the Neo-Geo which provided arcade-like experience was priced at US\$649.99\footnote{Adjusted to inflation the figure would be at of 2018 \$10,476 for a PC, \$377.00 for a SNES/Genesis, and \$1,134 for a Neo-Geo.}.
\end{itemize}
\par
...
A practical way to get an overview of the hardware available is to open up a PC and take a look at the component connecting everything together. In 1994, the best selling motherboard was the PX486P3 by QDI Computer, Inc\footnote{Again, a Canadian company \scaledimage{0.03}{canada} !}.\\
...
\par 
The black connectors show the traditional ISA bus extension ports. One 8-bit \circled{2} and three dual slot 16-bit \circled{3} allow four ISA cards. Also present, three connector of a new kind with an additional brown slot \circled{4}. These are VLB\footnote{a.k.a: VL-Bus, a.k.a: Video Local Bus.}, a new type of bus up to 10x faster than ISA.

\drawing{px486p3}{Components diagram of the PX486P3.}
\par
In the upper left \circled{6}, the main memory system had grown in capacity, speed and complexity. Thanks to a sharp decline in manufacturing price, the standard DRAM\footnote{Dynamic RAM.} installed would be 4 MiB\footnote{\doom{} would not run on PC equipped with only 2 MiB.}.\\
\par
...
% \end{enumerate}
\pagebreak
\trivia{Several of the most impressive game studio of the era speculated on the projected RAM price drop. Big titles of 1994 (including \doom) required a minimum of 4 MiB installed. Strike Commander, Ultima 8, and Comanche Maximum Overkill are prime examples.

\begin{center}
\scaledimage{0.31}{box_strike_commander.png} \scaledimage{0.31}{box_ultima8.png} \scaledimage{0.31}{box_comanche.png}
\end{center}
}
Ironically this time period would end up coinciding with the great RAM shortage of 1994 which saw the price go back up. The legend attributes the surge to a resin factory burning in Taiwan. In all likelihood the fluctuation was in fact due to the release of the new operating system by Microsoft, Windows 95 which recommended a machine with 8 MiB of RAM.
\begin{center}
\begin{tikzpicture}
