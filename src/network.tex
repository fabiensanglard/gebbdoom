...
\par The card had a BNC connector upon which was to be plugged a T-shaped connector. Each node was connected to up to two other nodes via a 10-Base-2 coaxial cables. There was no central point in this type of networking, all machines involved in the networked formed a chain. At both ends the chain had to be plugged signal terminator in order to prevent an RF signal from being reflected back from each end, causing interference, or power loss.
...
\end{wrapfigure}
The coaxial cable are bulky. Connecting an end to a T-Shape connector is fully achieved with a cool quarter turn of the coupling nut.\\
\par
...
\par
In figure \ref{10base5bncConnector} the four elements of a 1994 LAN. \circled{1} the T-Shaped connector connecting all \circled{2} coaxial cable, forming a link. Each ends of the chain must be closed with two load terminators \circled{3}. The network card connects to both the PC via its ISA bus extension slots (\circled{4}) and the LAN T-base slots.\\
\par
\trivia{Adding new machine on the network meant either unpluging one of the T-shaped connector or unplugging the chain terminator. In both cases, the central bus was broken and all other machines lost connectivity. Everybody remember this one friend who was always late to the LAN and forcing everybody to disconnect so he could join. The banwidth was shared and the theoretical 10Mb/s was often closer to 5Mb/s. This does not account for friends who wanted to exchange music in wav format (there was no mp3 at the time!).}\\
\par
...
\end{figure}
\par
\trivia{The fragility of these connection led to humorous way to end a message by typing "Hey! Wait! Don't pick up the ph\{\#`\%\$\{\%\&`+'\%\&NO CARRIER".}