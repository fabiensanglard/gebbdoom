...
\section{Maps}
Maps were designed in a top-down view. The only limitations were that walls were perpendicular to floors and both floors and walls were horizontal so everything was done in 2D. A designer worked with five types of elements: \cw{VERTEX}\footnote{Vertices coordinates were expressed with signed short integer [-32768, 32767]. 32 units translate to roughly one meter (or 3.28 feet for poor soul who must use the imperial system.)}, \cw{LINE}, \cw{SIDEDEF}, \cw{SECTOR}, and \cw{THING}.\\
\par
...
\par
Also notice the wall \cw{D-E} which mid-texture vertical offset was not correctly set, resulting in a vertical tear when connecting with wall \cw{E-F}. Wall \cw{B-C} vertical offset was properly set and result in no visual artifact. None of the wall use an horizontal offset, however the correspnoding field is labelled \cw{XOFF} on figure \ref{doom_map_basics} to show where its location.\\ 
\pagebreak
...
The very design of Objective-C also had a tremendous influence. The language's message dispatching system no-op behavior when dereferencing a \cw{nullptr}\footnote{"Understanding the Objective-C Runtime".} resulting in a fault forgiving environment where a faulty feature would not work but did not crash either.\\
\par   
...
The release of the source code in April 2015 allowed to peek inside. There is almost as much code as in the game engine (doom:32kloc, DoomED:20kcloc). Without the power of \NeXT the editor would have taken at least twice the same amount of time to make.
...
\par
\vspace{10pt}
\trivia{DoomED icon is an Imp. Upon starting up an Imp growling sound is played.}\\
\par
...
\par
\cfullimage{props/tom.png}{Tom Hall, delighted next to his NeXTCube running DoomED. The sticker on the monitor spells "quality".}
\par
...
\begin{tabularx}{\textwidth}{ L{0.15} L{0.75} }
  ...
  \toprule 
   ...
   \toprule
   \cw{BLOCKMAP} & 128x128 grid partition of the map \cw{LINEDEFS} to accelerate collision detections.\\
   \toprule
\end{tabularx}
